...
Cliff's Delta effect sizes are uniformly large (**δ ≥ 0.76**, well above the 0.47 large-effect threshold) across all baselines and output formats; **δ = 1.00** against five of six baselines indicates strict dominance. The advantage holds in a compute-controlled run (Caesar at $5/challenge with GPT-5-mini still tops Gemini 3 Deep Research) and in a blinded human A/B study, where 20 raters preferred Caesar's Cross-Domain Synthesis answer in 19 of 21 comparisons, 90.5% of the time (Holm-corrected p = 0.001). Ablations confirm both graph exploration and the adversarial verifier loop are independently necessary. See the [paper](https://arxiv.org/abs/2604.20855) for full methodology, exploration-budget ablation, and judge bias analysis.
