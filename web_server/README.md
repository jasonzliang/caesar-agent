...
Python version: 3.10–3.13 only. chromadb==1.5.2 uses pydantic.v1.BaseSettings,
which breaks on 3.14+. If python3 is 3.14, install 3.13 first
...
laptop). uvicorn binds :8090, Next.js binds :3000, both on 0.0.0.0.
...
All env vars are optional; defaults are sensible for a single-host demo.
    ...
    API_HOST              0.0.0.0   Backend bind addr. Forced to 127.0.0.1 with --password.
    ...
    OPENAI_API_KEY        required when CAESAR_DRY_RUN=0
...
## API endpoints
    ...
    GET    /runs?limit=N                              Recent runs, newest first.
...
full submit → poll → fetch loop in parallel in ~9s. Use
